...
#   -   PLAYER CLASS 	- 
	#position:
	pY: .word 4		# grid[3][4] STARTS AT (almost) THE MIDDLE
	pX: .word 3	
	direction: .word 0 #saving current direction, starting at 0 so it doesnt move initially
...
grid: .byte 'S','c','o','r','e',':','0',' '   #0
 ...
 pScore:  .word 0					#score initially set to 0
 GOreason: .asciiz "GAME OVERScore: "	#game over message, clumped together with score for easier access of printing
 
 
 #   -   DISPLAY CLASS 	-
 
 clear: .word 12		#clear ASCII 
	...
	j loadReward
		...
		loadReward: 
			...
			create_index:
				...
				beq $a0, 0,add2		#if random num is 0 add 2, and if 1 add 1, since 0 and 1 are off bounds (outside of wall), else continue.
				beq $a0, 1,addd
				j cont
				addd:
					addi $a0,$a0,1
		...
		loadScore:
			...
			twoDigit:
				...
				mul  $s0, $t1, 8
    				...
    				sb   $t8, 0($s0)          # storing first digit in score: x_
    				
    				#calculating index grid[7][0]
				li $t2, 7		
		...
		gameFunction:	
			...
			ifReward:
			 ...
			 #constant movement implementation
    				li $t0, 0xFFFF0000       # Receiver Control Register
    				lw $t1, 0($t0)           # Load control register
    				andi $t1, $t1, 1         # Check Ready bit (bit 0)
    				beq $zero, $t1, processDirection     # If not ready, go to proccess direction function

    				# Read key from MMIO keyboard data register
    				li $t0, 0xFFFF0004
    				lw $t1, 0($t0)           # Load pressed key
    				#checking new direction
    				
    
 			li   $t3, 'w'
			beq  $t1, $t3, newDir
			li   $t3, 's'
			beq  $t1, $t3, newDir			#loading wasd into temp registers to check if the direction is new, if it is jump to newDir
			li   $t3, 'a'
			beq  $t1, $t3, newDir
			li   $t3, 'd'
			beq  $t1, $t3, newDir	
 	
			j processDirection							# proccess direction
    	
    	
    			newDir:
    				sw $t1, direction			#storing new direction
    				
    			processDirection:
    			
    			lw $t2, direction			#loading direction
    			beq $zero,$t2, dontMove		#if direction == 0 (null)  then go to dontMove function
    			
    			li   $t3, 'w'
			beq  $t2, $t3, move_up
			li   $t3, 's'
			beq  $t2, $t3, move_down			#comparing new direction to loaded wasd and jumping into their associated functions to change/continue the current direction
			li   $t3, 'a'
			beq  $t2, $t3, move_left
			li   $t3, 'd'
			beq  $t2, $t3, move_right	
    				
    			dontMove:		#if the current direction is 0 (used as NULL) then continue at that direction (which is no direction)
    				j continueMove
			...
			continueMove:
				...
				li $a0, 100000  		#loading large number, which is the delay speed
        			delayLoop:
           			  addi $a0, $a0, -1		#looping by decreasing the number, until its zero, causing a delay.	
            		  bne $zero, $a0, delayLoop	
